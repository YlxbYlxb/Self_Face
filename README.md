...
1. **题库** —— 仓库内置 **1283 道**高频八股题，覆盖 Java 基础、集合、并发、JVM、Spring、MySQL、Redis、计算机网络、操作系统、算法、项目场景、**分布式与微服务**、**消息队列与性能优化**、**AI 与大模型**共 **14 个分类**。每题都有结构化的答题要点（不是一句话答案），并按难度（易/中/难）与高频标记分层。支持从 JSON 批量导入扩充，不必改代码。

   > 另有 1380 道扩展题库（Linux、C++、Go、测试开发 4 个新分类）从第三方 PDF 讲义提取，
   > **因许可限制不随仓库分发**，故上面只统计仓库内实际可用的题目。在本地持有该批 PDF 时，
   > 用 `scripts/import-interview-pdf.py` 即可复现，导入后共 2663 道 / 18 个分类。
...
│                                      另有提取类 seed 因许可限制不入库，已在 .gitignore 中排除）
...
├── scripts/                          质量门禁、题库提取与测试工具
│   ├── check_seed.py                 校验题库种子字段完整性
│   ├── import-javaguide.py           从开源文档批量生成题库（可复现的提取管道）
│   ├── import-interview-pdf.py       从 PDF 讲义提取题库（版面解析，规则全部固化）
│   ├── clean-figure-refs.py          清理答案里指向配图的悬空引用
...
├── pdf-inbox/                        放讲义 PDF 供提取脚本读取（已 gitignore，不进仓库）
